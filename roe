...
def load_config(project_root=None):
    default_config = {"src": "src", "build": "build", "target": "roe", "framework": "plain"}
...
def init_project(name=None, author=None, target=None, framework=None):
    ...
    if name:
        ...
        if project_root.exists():
            if project_root.is_dir():
                if any(project_root.iterdir()):
                    ...
                    print("💡 Choose a different project name or remove the existing directory")
                    exit(1)
            else:
                print(f"❌ Error: A file named '{name}' already exists")
                print(f"📁 Location: {project_root}")
                print("💡 Choose a different project name")
                exit(1)
        
        # Create the project directory
        project_root.mkdir(parents=True, exist_ok=True)
        print(f"📁 Created project directory: {name}/")
    else:
        ...
        if any(project_root.iterdir()):
            config_path = project_root / "roeconfig.json"
            if config_path.exists():
                print("❌ Error: This directory already contains a Roelang project")
                print(f"📁 Current directory: {project_root}")
                print("💡 Use a new empty directory to create a different project")
            else:
                print("❌ Error: Directory is not empty")
                print(f"📁 Current directory: {project_root}")
                print("💡 'roe init' must be run in an empty directory or provide a project name")
            exit(1)
    ...
    if framework:
        ...
        target_key = target or "roe"
        if target_key in valid_frameworks:
            if framework != "plain" and framework not in valid_frameworks[target_key]:
                print(f"❌ Error: Framework '{framework}' is not compatible with target '{target_key}'")
                if valid_frameworks[target_key]:
                    print(f"💡 Valid frameworks for {target_key}: {', '.join(valid_frameworks[target_key])}")
                else:
                    print(f"💡 Target '{target_key}' does not support frameworks")
                exit(1)
        
        # Add framework if specified and applicable (ignore for targets that don't support frameworks)
        if framework != "plain" and target_key in ["java", "python", "go", "node", "rust", "roe"]:
...
'''
        ...
        print("✅ Created src/main.roe")

    print("✅ Initialized Roelang project successfully!")
    print(f"📁 Project name: {name or project_root.name}")
    if author:
    ...
    print(f"🎯 Target: {target or 'roe'}" + (f" with {framework} framework" if show_framework else ""))
    print("\n📂 Directory structure:")
...
def install_package(package_name, is_dev=False):
    ...
    if not package_path.exists():
        print("❌ Error: package.json not found. Run 'roe init' first.")
        exit(1)
...
''')
        ...
        print(f"✅ Downloaded math-utils to modules/{package_name}")
...
''')
        ...
        print(f"✅ Downloaded string-utils to modules/{package_name}")
    else:
        print(f"❌ Package '{package_name}' not found in registry")
        print("💡 Available packages: math-utils, string-utils")
        exit(1)
    ...
    print(f"✅ Added {package_name} to {dep_key}")
    print(f"💡 Use: include \"{package_name}/{package_name.replace('-', '_')}.roe\"")
...
def clean_build(clean_all=False):
    ...
    if build_dir.exists():
        ...
        print(f"ℹ️  Build directory {build_dir} doesn't exist")
    ...
    if clean_all:
        if dist_dir.exists():
            ...
            print(f"ℹ️  Dist directory {dist_dir} doesn't exist")
    
    if cleaned_dirs:
        print(f"✅ Cleaned {len(cleaned_dirs)} director{'y' if len(cleaned_dirs) == 1 else 'ies'}: {', '.join(cleaned_dirs)}")
    else:
...
def find_project_root():
    ...
    print(f"❌ Error: No roeconfig.json found (searched up {depth} directories)")
    print(f"📂 Current directory: {original}")
    print(f"\n💡 To create a Roelang project, run: roe init")
...
def build_distribution(project_root, clean=False, release=False):
    ...
    config_path = project_root / "roeconfig.json"
    if not config_path.exists():
        print("❌ Error: roeconfig.json not found")
        exit(1)
    ...
    print(f"📦 Building distribution package...")
    print(f"🎯 Target: {target}" + (f" ({framework})" if framework != "plain" else ""))
    if release:
    ...
    if not roe_files:
        print(f"⚠️  Warning: No .roe files found in {search_root}")
        print("💡 Add some .roe files to your project first")
    else:
        print(f"🔍 Found {len(roe_files)} .roe files to compile:")
        for roe_file in roe_files:
            rel_path = roe_file.relative_to(project_root)
            print(f"   {rel_path}")
        
        # Compile all .roe files
        print(f"🔨 Compiling {len(roe_files)} files...")
        compiled_count = 0
        failed_count = 0
        ...
        for roe_file in roe_files:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    failed_count += 1
                    print(f"   ❌ {rel_path}: {result.stderr.strip()}")
            except Exception as e:
                failed_count += 1
                print(f"   ❌ {rel_path}: {e}")
        ...
        if failed_count > 0:
            print("⚠️  Some files failed to compile. Distribution will only include successful compilations.")
    ...
    if target == "java" and framework == "spring":
        ...
        print("📱 Packaging Spring Boot projects...")
        ...
        for project_dir in build_dir.iterdir():
            if project_dir.is_dir() and (project_dir / "pom.xml").exists():
                ...
                print(f"📋 Copied Spring Boot project: {relative_path}")
        ...
        if not copied_files:
            print("⚠️  Warning: No Spring Boot projects found in build directory")
            print("💡 Run 'roe compile' on your .roe files first")
    
    elif target == "mobile":
        # Handle mobile projects
        print("📱 Packaging mobile projects...")
        ...
        for mobile_dir in ["android", "ios"]:
            ...
            if mobile_path.exists():
                ...
                print(f"📋 Copied mobile project: {mobile_dir}")
        
        if not copied_files:
            print("⚠️  Warning: No mobile projects found in build directory")
            print("💡 Run 'roe compile' on your .roe files first")
        ...
        pattern = target_extensions.get(target, "*.roebc")
        print(f"📦 Looking for {pattern} files...")
        ...
        if not copied_files:
            print(f"⚠️  Warning: No {pattern} files found in build directory")
            print("💡 Run 'roe compile' or 'roe run' on some files first")
    ...
    print(f"✅ Distribution built successfully!")
    print(f"📦 {len(copied_files)} files copied to {dist_dir}")
    if main_entry:
...
def main():
    ...
    init_parser.add_argument('--target', choices=['wasm', 'python', 'java', 'go', 'node', 'html', 'mobile', 'rust', 'roe'], help='Compilation target')
    ...
    compile_parser.add_argument('--target', choices=['wasm', 'python', 'java', 'go', 'node', 'html', 'kotlin', 'swift', 'bytecode', 'mobile', 'rust', 'roe'], help='Compilation target (overrides roeconfig.json)')
    ...
    if len(sys.argv) >= 2 and sys.argv[1] not in ['init', 'install', 'build', 'clean', 'compile', 'run', '--version', '-h', '--help']:
        if sys.argv[1] in ['-h', '--help']:
            ...
            print("❌ Unknown command. Use 'roe --help' for usage information.")
            return
...
def compile_file(roe_filename, output_format='wasm', output_path=None, watch=False, target_override=None):
    ...
    def do_compile():
        ...
        try:
            ...
            if target_override:
                ...
                config_target = config.get("target", "roe")
            ...
            print(f"🔨 Compiling {roe_file} -> {target.upper()}")
            ...
            if hasattr(compiler, 'compile_file') and hasattr(compiler, 'CompilerError'):
                try:
                    ...
                    if isinstance(output_file, dict) and 'files' in output_file:
                        ...
                        print(f"✅ Compiled successfully -> {project_root}/ ({num_files} files)")
                    else:
                        # Single file result
                        print(f"✅ Compiled successfully -> {output_file}")
                    
                    # Runtime files are no longer needed - using inline code generation
                    print("ℹ️  Runtime dependency eliminated - using native language features")
                    ...
                    if target == 'wasm' and output_format == 'wasm' and isinstance(output_file, str):
                        ...
                        if output_file.endswith('.wat'):
                            ...
                            if result.returncode != 0:
                                print(f"❌ Error compiling to WASM: {result.stderr}")
                                return False
                            print(f"✅ Also generated -> {wasm_output}")
                ...
                print("⚠️  Using legacy WAT-only compiler")
                ...
                if output_format == 'wasm':
                    ...
                    if result.returncode != 0:
                        print(f"❌ Error compiling to WASM: {result.stderr}")
                        return False
                    print(f"✅ Compiled successfully -> {wasm_file}")
                else:
                    print(f"✅ Compiled successfully -> {wat_file}")
    ...
    if watch:
        print(f"👀 Watching {roe_filename} for changes...")
        roe_file = Path(roe_filename)
        if not roe_file.is_absolute():
            roe_file = Path.cwd() / roe_filename
...
def run_bytecode_file(roe_filename):
    ...
    if not vm_binary.exists():
        print(f"❌ Error: Roe VM not found at {vm_binary}")
        print("💡 You may need to build the VM: cargo build --release --manifest-path roevm/Cargo.toml")
        return False
    
    print(f"🚀 Running {bytecode_file} with Roe VM")
    print("─" * 40)
    ...
    try:
        ...
        if results:
            if release:
                ...
                print(f"\n✅ Development builds completed!")
        else:
            print(f"\n❌ No builds were successful")
            print(f"💡 Check that mobile projects exist in build/ directory")
            exit(1)
            
    except ImportError as e:
        print(f"❌ Mobile build system not available: {e}")
        print(f"💡 Make sure the Roelang compiler is properly installed")
        exit(1)
    except Exception as e:
        print(f"❌ Mobile build failed: {e}")
        exit(1)
...
def run_mobile_app(roe_filename, watch=False):
    ...
    def do_mobile_run():
        ...
        try:
            ...
            print(f"❌ Failed to run mobile app: {e}")
            return False
